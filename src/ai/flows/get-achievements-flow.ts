...
const AchievementSchema = z.object({
  ...
  icon: z.enum(['Target', 'Trophy', 'Zap', 'Star', 'Lock']).describe('The icon to display for the achievement.'),
});
...
const prompt = ai.definePrompt({
  ...
  - Peak Hour Deliveries (this week): {{{peakHourDeliveries}}}
  - Weekend Deliveries (last weekend): {{{weekendDeliveries}}}
  - Late Night Deliveries (this month): {{{lateNightDeliveries}}}
  - Overall Rating: {{{overallRating}}}/5
  - 5-Star Rating Streak: {{{fiveStarRatingStreak}}}

  Generate a list of 5-6 relevant achievements. The achievements should be a mix of short-term and long-term goals.
  ...
  - Ensure the 'icon' matches the achievement's theme. Use 'Target' for delivery count goals, 'Trophy' for major milestones, 'Zap' for time-based goals like peak hours, 'Star' for rating goals, and 'Lock' for locked achievements.
  ...
  - "Peak Hour Power": Target: 5 (for the week), Progress: {{{peakHourDeliveries}}}
  - "Weekend Warrior": Target: 25 (for the weekend), Progress: {{{weekendDeliveries}}}
  - "Night Owl": Target: 10 (for the month), Progress: {{{lateNightDeliveries}}}. Mark as Locked if progress is 0.
  - "Delivery Pro": Target: 100 total deliveries, Progress: {{{totalDeliveries}}}

  Return a list of achievements in the specified format.
  `,
});
